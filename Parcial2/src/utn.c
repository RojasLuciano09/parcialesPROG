...
int esUnArchivo(char* cadena)
{
	...
	if(cadena != NULL){
		for(i=0;cadena[i] != '\0';i++){
			if((cadena[i] != ' ') && (cadena[i] < 'a' || cadena[i] > 'z') && (cadena[i] < 'A' || cadena[i] > 'Z') && (cadena[i] < '0' || cadena[i] > '9') && (cadena[i] == 45 || cadena[i] ==46 ) && (cadena[i] == 95 || cadena[i] ==64 ) ){
				output = 0;
				break;
			}
		}
...
int utn_getFileName(char* pMensaje, char* pMensajeError, char* pResultado, int reintentos, int limite)
{
	...
	if(pMensaje != NULL && pMensajeError != NULL && pResultado != NULL && reintentos >= 0 && limite > 0)
	{
		...
		{
			printf("%s", pMensaje);
			if(	(myGets(bufferString, ARRAY_SIZE)== 0) &&
...
int utn_getIntStr(char msg[], char msgError[], char pResult[], int attemps, int limit)
{
	...
	if(msg != NULL && msgError != NULL && pResult != NULL && attemps >= 0)
	{
		...
		{
			printf("\n%s", msg);
			if(myGets(bufferString, ARRAY_SIZE) == 0 && isInt(bufferString) == 1 && strnlen(bufferString, sizeof(bufferString)) < limit)
			{
				retorno = 0;
				strncpy(pResult, bufferString, limit);
				break;
